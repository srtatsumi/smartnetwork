<div id="sidebar-menu" class="main_menu_side hidden-print main_menu">
    <div class="menu_section">
    <h3>Dashboard</h3>
    <ul class="nav side-menu">
        
        <li>
            <a href="index.php">
                <i class="fa fa-home"></i> Home
            </a>
        </li>
        <!-- <li> -->
        <!-- <form>
    <button style="background-color: transparent;
  color: white;
  padding: 5px 15px;
  text-decoration: none;
  cursor: pointer;
  border: none;" type="submit" formaction="https://rzp.io/l/8usToKi">Pay</button>
  </form>
        </li> -->
        <li>
           <a href="watchad.php">
               <i class="fa fa-adn"></i>Watch Ads
           </a>
        </li>
        <li>
           <a href="https://rzp.io/l/8usToKi">
               <i class="fa fa-money"></i>Pay
           </a>
        </li>
        <li data-bs-toggle="modal" data-bs-target="#exampleModal">
            <a href="javascript:;">
                <i class="fa fa-clone"></i>Update Transaction ID
            </a>
        </li>
        <li data-bs-toggle="modal" data-bs-target="#exampleModalMoney" onclick='amountCheck()'>
            <a href="javascript:;">
                <i class="fa fa-inr"></i>My Wallet
            </a>
        </li>
        <li data-bs-toggle="modal" data-bs-target="#exampleBank">
            <a href="javascript:;">
                <i class="fa fa-inr"></i>Add Bank Details
            </a>
        </li>
        <li>
            <a href="transactions.php">
                <i class="fa fa-clone"></i>Transactions
            </a>
        </li>
        <li data-bs-toggle="modal" data-bs-target="#exampleModal">
            <a href="./change_password.php">
                <i class="fa fa-key"></i>Change Password
            </a>
        </li>
        
    </ul>
     
    </div>
</div>